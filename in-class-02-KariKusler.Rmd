...
```
The command re-names the columns.
...
rain.df$daily <- rwoSums(rain.df,na.rm=TRUE)
...
rain$daily <- mutate(apply(rowsums,rain.tbl,c[,1:23]))
head(rain$daily)

apply(rain.df,2,sum,1:23)


mutate(rain.df, daily = sum(1:24))
rain.df$daily <- sum(rain.df,1:24)
head(rain.df)
```
